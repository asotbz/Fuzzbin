...
class VideoBase(BaseModel):
    ...
    artist: Optional[str] = Field(
        default=None, max_length=255, description="Primary artist name"
    )
    album: Optional[str] = Field(default=None, max_length=255, description="Album name")
    year: Optional[int] = Field(
        default=None, ge=1900, le=2100, description="Release year"
    )
    director: Optional[str] = Field(
        default=None, max_length=255, description="Video director"
    )
    genre: Optional[str] = Field(default=None, max_length=100, description="Music genre")
    studio: Optional[str] = Field(
        default=None, max_length=255, description="Production studio"
    )
...
class VideoCreate(VideoBase):
    ...
    # External IDs
    imvdb_video_id: Optional[str] = Field(
        default=None, max_length=50, description="IMVDb video ID"
    )
    youtube_id: Optional[str] = Field(
        default=None, max_length=50, description="YouTube video ID"
    )
    vimeo_id: Optional[str] = Field(
        default=None, max_length=50, description="Vimeo video ID"
    )

    # File paths (optional during creation)
    video_file_path: Optional[str] = Field(
        default=None, description="Absolute path to video file"
    )
    nfo_file_path: Optional[str] = Field(
        default=None, description="Absolute path to NFO file"
    )
...
class VideoFilters(BaseModel):
    ...
    artist: Optional[str] = Field(
        default=None, description="Filter by artist name (partial match)"
    )
    album: Optional[str] = Field(
        default=None, description="Filter by album name (partial match)"
    )
    director: Optional[str] = Field(
        default=None, description="Filter by director (partial match)"
    )
    genre: Optional[str] = Field(
        default=None, description="Filter by genre (partial match)"
    )

    # Exact match filters
    status: Optional[VIDEO_STATUSES] = Field(
        default=None, description="Filter by status"
    )
    year: Optional[int] = Field(default=None, description="Filter by exact year")
    year_min: Optional[int] = Field(
        default=None, ge=1900, description="Filter by minimum year"
    )
    year_max: Optional[int] = Field(
        default=None, le=2100, description="Filter by maximum year"
    )

    # External ID filters
    imvdb_video_id: Optional[str] = Field(
        default=None, description="Filter by IMVDb ID"
    )
    ...
    search: Optional[str] = Field(
        default=None, description="Full-text search query (FTS5)"
    )

    # Soft-delete handling
    include_deleted: bool = Field(
        default=False, description="Include soft-deleted videos"
    )
