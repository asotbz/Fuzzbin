...
class SpotifyAlbum(BaseModel):
    ...
    images: List[Dict[str, Any]] = Field(
        default_factory=list, description="Album artwork images"
    )
...
class SpotifyTrack(BaseModel):
    ...
    artists: List[SpotifyArtist] = Field(
        default_factory=list, description="Track artists"
    )
    album: Optional[SpotifyAlbum] = Field(default=None, description="Album information")
    duration_ms: Optional[int] = Field(
        default=None, description="Track duration in milliseconds"
    )
    popularity: Optional[int] = Field(
        default=None, description="Popularity score 0-100"
    )
...
class SpotifyPlaylistTrack(BaseModel):
    ...
    added_by: Optional[Dict[str, Any]] = Field(
        default=None, description="User who added track"
    )
...
class SpotifyPlaylist(BaseModel):
    ...
    collaborative: Optional[bool] = Field(
        default=None, description="Collaborative flag"
    )
