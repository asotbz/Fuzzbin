...
class TagsSet(BaseModel):
    ...
    tags: List[str] = Field(..., description="List of tag names to apply (will create if needed)")
    source: Literal["manual", "auto"] = Field(default="manual", description="Tag source")


class TagsAdd(BaseModel):
    """Schema for adding tags to a video (keeps existing)."""

    tags: List[str] = Field(..., description="List of tag names to add (will create if needed)")
    source: Literal["manual", "auto"] = Field(default="manual", description="Tag source")
