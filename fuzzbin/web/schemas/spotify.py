...
These schemas mirror the models from fuzzbin.parsers.spotify_models,
...
class SpotifyTrackSchema(BaseModel):
    """Spotify track information."""

    id: str = Field(description="Spotify track ID")
    name: str = Field(description="Track name")
    uri: str = Field(description="Spotify URI")
    artists: List[SpotifyArtistSchema] = Field(default_factory=list, description="Track artists")
    album: Optional[SpotifyAlbumSchema] = Field(default=None, description="Album information")
...
class SpotifyPlaylistTrackSchema(BaseModel):
    """Playlist track item (wraps track with added_at, added_by)."""

    track: SpotifyTrackSchema = Field(description="Track object")
...
class SpotifyPlaylistSchema(BaseModel):
    ...
    tracks: Optional[SpotifyPlaylistTracksResponseSchema] = Field(
...
class SpotifyAllTracksResponseSchema(BaseModel):
    """Response containing all tracks from a playlist."""

    playlist_id: str = Field(description="Spotify playlist ID")
    total: int = Field(description="Total number of tracks")
    tracks: List[SpotifyTrackSchema] = Field(
        default_factory=list, description="All tracks in the playlist"
    )
