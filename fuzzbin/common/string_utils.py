...
def normalize_genre(genre: str) -> Tuple[str, str, bool]:
    ...
    .. deprecated::
        Use classify_single_genre() from genre_buckets module directly.
        This function is retained for backward compatibility.

    Uses regex-based bucket classification to map genres/styles to one of the
    primary categories: Metal, Hip Hop/R&B, Country, Pop, Electronic, Rock.

    Unmapped genres pass through unchanged, preserving specificity.

    Args:
        genre: Genre string from Discogs, Spotify, or other source
    ...
    Returns:
        ...
        - is_mapped: True if genre was mapped to a bucket
    ...
    """
    from .genre_buckets import classify_single_genre

    original = genre.strip()
    bucket, _ = classify_single_genre(original)

    if bucket:
        return (original, bucket, True)
...
def get_primary_genre_categories() -> List[str]:
    """
    Get the list of primary genre categories (bucket names).

    Returns:
        List of primary genre category names in priority order

    Example:
        >>> get_primary_genre_categories()
        ['Metal', 'Hip Hop/R&B', 'Country', 'Pop', 'Electronic', 'Rock']
    """
    from .genre_buckets import PRIORITY
